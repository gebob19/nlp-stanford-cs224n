...
class NMT(nn.Module):
    ...
    def encode(self, source_padded: torch.Tensor, source_lengths: List[int]) -> Tuple[torch.Tensor, Tuple[torch.Tensor, torch.Tensor]]:
        ...
        X = self.model_embeddings.source(source_padded) # embed sentences
        # pack s.t |nth batch| == |sentences whose nth word is not padding| 
        # visualization :
    ...
    def step(self, Ybar_t: torch.Tensor,
        ...
        e_t = torch.bmm(enc_hiddens_proj, dec_hidden.unsqueeze(-1))
        ...
        a_t = torch.bmm(enc_hiddens.permute(0, 2, 1), alpha_t).squeeze(-1)
