...
class VocabEntry(object):
    ...
    def words2charindices(self, sents):
        ...
        for s in sents:
            sent = []
            for w in s:
                chars = [self.start_of_word] + [self.char2id[c] for c in w] + [self.end_of_word]
                sent.append(chars)
            char_ids.append(sent)
    ...
    def to_input_tensor_char(self, sents: List[List[str]], device: torch.device) -> torch.Tensor:
        ...
        sents_t = pad_sents_char(char_ids, self.char2id['<pad>']) # (batch_size, max_sentence_length, max_word_length)
